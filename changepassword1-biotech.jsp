...
<%@page import="java.io.*"%>
 <left> <a href='index_biotech.jsp'>GoBack</a></left>
...
try{
  ...
  rs=stmt.executeQuery("select username FROM tablemasterstaff WHERE username='placementbiotech'");
    ...
    while(rs.next())
       {
           ...
           //out.println(Que+"     "+Ans);
                       if(uname.equalsIgnoreCase("placementbiotech"))
          {
              
             //rs=stmt.executeQuery("update TABLEMASTER set PASSWORD='"+conpass+"' WHERE REGISTERNO='"+RN1+"'");
             //out.println(RN1);
              String update="update tablemasterstaff set PASSWORD=? WHERE username='placementbiotech'";
