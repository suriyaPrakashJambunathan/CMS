<%@ page language="java" %>
<%
String username1=(String) session.getAttribute("username4");
...
rs = st.executeQuery("select * from TABLEMASTER_EXAM where EID ='"+username1+"'");
while(rs.next()){ %>
 <tr>
...
<td><input type="text" STYLE="background-color:#FFFFE0;border:10px solid #FFFFE0;color: #BDB76B; font-family: Verdana; font-weight: bold; font-size: 20px;" size="22" maxlength="30" name="DIP" disabled value="<%= rs.getString(10) %>"></td></tr>
  ...
  </td><td><a href='edit1.jsp?id=<%=rs.getString(1)%>'><h3>EDIT</h3></a></td>
