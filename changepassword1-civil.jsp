...
<%@page import="java.io.*"%>
 <left> <a href='index_civil.jsp'>GoBack</a></left>
...
try{
  ...
  rs=stmt.executeQuery("select username FROM tablemasterstaff WHERE username='placementcivil'");
    ...
    while(rs.next())
       {
           ...
           //out.println(Que+"     "+Ans);
                       if(uname.equalsIgnoreCase("placementcse"))
          {
              
             //rs=stmt.executeQuery("update TABLEMASTER set PASSWORD='"+conpass+"' WHERE REGISTERNO='"+RN1+"'");
             //out.println(RN1);
              String update="update tablemasterstaff set PASSWORD=? WHERE username='placementcivil'";
